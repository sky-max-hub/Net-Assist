...
- [x] 5.1 手动验证：创建多个 tab → 关闭应用 → 重新打开 → 确认 tab 恢复且状态为 idle
- [x] 5.2 手动验证：在 TCP Client 中发送/接收消息 → 点击清空 → 确认消息清空且其他 tab 不受影响
- [x] 5.3 手动验证：新建 TCP Client → 确认 host 默认显示 127.0.0.1
